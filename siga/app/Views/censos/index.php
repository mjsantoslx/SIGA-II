...
<?php if (empty($anos)): ?>
    <p class="texto-vazio">Ainda não existem anos escotistas definidos. <?= \App\Core\Sessao::ehAdministrador() ? '<a href="/admin/anos-escotistas/criar">Criar o primeiro</a>.' : 'Peça a um administrador para os definir.' ?></p>
<?php else: ?>

<form action="/censos" method="get" class="formulario-filtros">
            ...
            <?php foreach ($linhas as $linha): ?>
                <tr>
                    ...
                    <td class="tabela-acoes">
                        <?php if ($linha['Pago']): ?>
                            <form action="/censos/<?= (int) $linha['IdAssociado'] ?>/<?= (int) $anoSeleccionado ?>/nao-pago" method="post" class="forma-inline" onsubmit="return confirm('Marcar como não pago?');">
                                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                                <button type="submit" class="ligacao-botao">Marcar não pago</button>
                            </form>
                        <?php else: ?>
                            <form action="/censos/<?= (int) $linha['IdAssociado'] ?>/<?= (int) $anoSeleccionado ?>/pago" method="post" class="linha-contacto-editar" style="border: none; padding: 0;">
                                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                        ...
                        <?php endif; ?>
                        <a href="/censos/<?= (int) $linha['IdAssociado'] ?>/<?= (int) $anoSeleccionado ?>/historico" style="margin-left: 0.6rem; font-size: 0.85rem;">Histórico</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
